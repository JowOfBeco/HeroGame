...
root.style.setProperty('--tile-size', `${TILE_SIZE}px`); //48 px
root.style.setProperty('--helmet-offset', `${HELMET_OFFSET}px`); //12 px
root.style.setProperty('--game-size', `${GAME_SIZE}px`); //960PX

// ---

function createBoard() {
    const boardElement = document.getElementById('board'); // ID do item board no index.html
    const elements = []
    ...
    function createElement(options) {
      ...
      function getMovementConflict(position, els) {
        const conflictItem = els.find((currentElement) => {
            return(currentElement.currentPosition.top === position.top &&
    ...
    function createEnemy(options) {
        ...
        setInterval(() => {
            ...
            const randomIndex = Math.floor(Math.random() * direction.length) //Math.floor faz o numero ficar inteiro
            const randomDirect = direction[randomIndex]

            enemy.move(randomDirect)
        }, 1000)
    }
...
board.createEnemy({  top: TILE_SIZE * 8, left: TILE_SIZE * 10  });
board.createEnemy({  top: TILE_SIZE * 12, left: TILE_SIZE * 1  });
board.createEnemy({  top: TILE_SIZE * 5, left: TILE_SIZE * 8  });
board.createEnemy({  top: TILE_SIZE * 5, left: TILE_SIZE * 15  });
board.createEnemy({  top: TILE_SIZE * 5, left: TILE_SIZE * 15  });
board.createEnemy({  top: TILE_SIZE * 5, left: TILE_SIZE * 15  });
board.createEnemy({  top: TILE_SIZE * 5, left: TILE_SIZE * 15  });
...
board.createItem({ item:'trap', top: TILE_SIZE * 10, left: TILE_SIZE * 16 });
board.createItem({ item:'trap', top: TILE_SIZE * 5, left: TILE_SIZE * 5 });
board.createItem({ item:'trap', top: TILE_SIZE * 15, left: TILE_SIZE * 5 });
